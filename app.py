...
# =========================================================
# MISTER TRUCO - ARQUITETURA ESTÁTICA HOMOLOGADA
# Baseado na lógica de estados dos motores do GitHub
# =========================================================

st.set_page_config(page_title="Mister Truco Pro", page_icon="🃏", layout="wide")

# --- CUSTOMIZAÇÃO VISUAL (TAPETE VERDE RETRÔ) ---
st.markdown("""
    <style>
...
def process_trick_victory(g: dict):
    powers = [get_card_power(p["card"], g["mode"], g["vira"]) for p in g["table"]]
    max_pow = max(powers)
    winners = [i for i, pow_val in enumerate(powers) if pow_val == max_pow]
    
    is_tie = False
    if len(winners) > 1:
        teams = {g["table"][w_idx]["player"] % 2 for w_idx in winners}
        if len(teams) > 1: is_tie = True
        
    if is_tie:
        g["trick_wins"].append(-1)
        next_puller = g["table"][winners[0]]["player"]
        g["message"] = "A vaza empatou! Quem amarrou joga primeiro."
    else:
        winning_play = g["table"][powers.index(max_pow)]
    ...
    if hand_winner is not None:
        ...
        g["message"] = f"Fim de rodada! Time {hand_winner + 1} faturou {g['hand_points']} ponto(s)."
...
def bot_play(g: dict):
    ...
    if not g["pending_truco"] and g["hand_points"] < 12 and random.random() < 0.05:
        next_val = {1:3, 3:6, 6:9, 9:12}.get(g["hand_points"], 12)
        g["pending_truco"] = {"from_team": g["turn"] % 2, "to_team": 0, "value": next_val}
        g["message"] = f"A CPU pediu {next_val if next_val > 3 else 'TRUCO'}! Aceita ou corre?"
        return
    ...
    if not g["table"]:
        ...
        table_max = max([get_card_power(p["card"], g["mode"], g["vira"]) for p in g["table"]])
    ...
    g["table"].append({"player": g["turn"], "card": card})
...
# Inicialização com proteção absoluta de dados nulos
...
if g["pending_truco"] and g["pending_truco"]["to_team"] == 1:
    if random.random() < 0.6:
        ...
        g["pending_truco"] = None
        g["message"] = f"A CPU aceitou! Jogo valendo {g['hand_points']} pontos."
    else:
        g["score"][0] += g["hand_points"]
        start_hand(g)
        g["message"] = "A CPU correu! Você ganhou os pontos."
    st.rerun()
...
with col_board:
    cpu_left = len(g["hands"][1]) if g["n_players"] == 2 else len(g["hands"][1]) + len(g["hands"][3])
    cols_cpu = st.columns([1, 1, 1, 1, 1])
    for i in range(min(cpu_left, 3)):
        with cols_cpu[i + 1]:
    ...
    for idx, card in enumerate(g["hands"][0]):
        with cols_player[idx + 1]:
            ...
            if st.button("Jogar", key=f"btn_c_{idx}_{card.label}", disabled=not my_turn_active, use_container_width=True):
                g["table"].append({"player": 0, "card": g["hands"][0].pop(idx)})
                if len(g["table"]) == g["n_players"]:
                    process_trick_victory(g)
                else:
                    g["turn"] = (g["turn"] + 1) % g["n_players"]
    ...
    with cols_act[1]:
        if st.button(truco_label, key="act_truco_main", disabled=not next_bet or not my_turn_active, use_container_width=True):
            next_val = {1:3, 3:6, 6:9, 9:12}.get(g["hand_points"])
            g["pending_truco"] = {"from_team": 0, "to_team": 1, "value": next_val}
            g["message"] = "Você pediu aumento! Aguardando a CPU..."
            st.rerun()
    with cols_act[2]:
        if st.button("CORRER", key="act_fold_main", disabled=not my_turn_active, use_container_width=True):
...
with col_panel:
    ...
    if g["pending_truco"] and g["pending_truco"]["to_team"] == 0:
        ...
        c_ch = st.columns(2)
        if c_ch[0].button("ACEITAR", key="acc_ch_f", use_container_width=True):
            g["hand_points"] = g["pending_truco"]["value"]
            g["pending_truco"] = None
            g["message"] = f"Aceito! Jogo valendo {g['hand_points']}."
            st.rerun()
        if c_ch[1].button("CORRER", key="fold_ch_f", use_container_width=True):
            g["score"][1] += g["hand_points"]
            start_hand(g)
            st.rerun()
    ...
    if not g["table"]:
        ...
        for play in g["table"]:
            c = play["card"]
            p_name = "Você" if play["player"] == 0 else f"Bot {play['player']}"
            st.markdown(f"""
    ...
    if st.button("Avançar Próxima Mão", key="next_hand_f", disabled=not g["finished_hand"], use_container_width=True):
        start_hand(g)
        st.rerun()
